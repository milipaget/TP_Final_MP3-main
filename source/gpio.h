...
/**
 * @brief Configures all digital filter parameters from a specified digital pin.
 * @param pin the pin to read (according PORTNUM2PIN)
 * @param value HIGH = filter enabled, LOW = filter disabled
 * @param clockInput LOW = bus clock, HIGH = LPO 1kHz clock
 * @param ticks integer value from 0 to 31
 * @return HIGH if the ticks parameter is correct, LOW otherwise
 */
